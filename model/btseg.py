...
class ResNetBlock(nn.Module):
    def __init__(self, channels):
        ...
        #
        self.feats = nn.Sequential(nn.GroupNorm(32, channels_in),
                                   nn.ReLU(inplace=True),
                                   nn.Conv3d(channels_in, channels_out,
                                       kernel_size=3, stride=1, padding=1),
                                   nn.GroupNorm(32, channels_in),
                                   nn.ReLU(inplace=True),
                                   nn.Conv3d(channels_out, channels_out,
                                       kernel_size=3, stride=1, padding=1))
...
class BraTSSegmentation(nn.Module):
    def __init__(self):
        ...
        self.initConv = nn.Conv3d(4, 32, kernel_size=3, stride=1, padding=1)
